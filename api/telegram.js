...
export default async function handler(req, res) {
  ...
  if (update.callback_query) {
    ...
    if (data.startsWith('cat:')) {
      ...
      if (ok) {
        const valorFmt = d.valor ? `R$ ${parseFloat(d.valor).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}` : '—';
        const [ano, mes, dia] = (d.data || '').split('-');
        const dataFmt = d.data ? `${dia}/${mes}/${ano}` : '—';
        await sendMessage(chatId,
  ...
  if (texto === '/start' || texto === '/ajuda') {
    await sendMessage(chatId,
      `👋 *Bot Financeiro MC*\n\n` +
      `Descreva o pagamento em texto e eu registro no sistema.\n\n` +
      `*Exemplos:*\n` +
      `• \`R$ 1.200 aluguel galpão 08/05\`\n` +
      `• \`500 embalagem ontem\`\n` +
      `• \`R$ 3.800,00 fornecedor Martins hoje\`\n` +
      `• \`1500 honorários contador 05/05/2026\`\n\n` +
      `_Valor + descrição + data (opcional)_`
    );
    return res.status(200).end();
  }
  ...
  const valorFmt = extraido.valor ? `R$ ${parseFloat(extraido.valor).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}` : '?';
  const [ano, mes, dia] = (extraido.data || '').split('-');
  const dataFmt = extraido.data ? `${dia}/${mes}/${ano}` : '?';
